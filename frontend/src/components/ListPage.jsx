...
export function ListRow({ primary, description, amount, trailing }) {
  return (
    <div className="flex items-center gap-3.5 py-[13px] border-b border-rule-soft text-sm">
      <span className="font-semibold min-w-[190px]">{primary}</span>
      <span className="text-ink-muted flex-1">{description}</span>
...
function ListPage({ title, blurb, actions, children }) {
  ...
  return (
    <main className="px-[30px] pt-3.5 pb-[34px] flex flex-col gap-4 flex-1 h-full">
